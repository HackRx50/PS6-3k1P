...
async def gen_script_and_choose_vid(pdf_content, n):
    ...
    n= n/10
    prompt = '''Think of yourself as an expert script writter for compeling social media video\n\nContent:''' + pdf_content + "\n\n" + f'''From the content, make script for an interesting video where a video will support each subscript. The description of the videos are as following. {chosen_description}. The scripts must have a storyline and be written keeping in mind the description of video. Do not use vid description to write the script, just use it to choose. The narrative should mention the product as the one that solves the problem.  Use simpler language, make it sound more natural like someone is narrating a story. There should be ONLY {n} scripts and chosen videos. Script should always be less than 25 words. Choose the videos in any order. The subscripts must collectively include all the important information in content for any customer. The answer should contain the subscript to be spoken and corresponding chosen vid. Don't choose any video more than once Format the answer only as a list of json objects with just 2 key called Subscript and Video. Only give the json. No emojis. '''
    ans = await chat_completion(prompt)
    ans = ans.strip("```")
